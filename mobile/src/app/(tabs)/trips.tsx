      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        <View className="px-6 py-6">
          <View className="flex-row items-center justify-between mb-6">
            <Text className="text-2xl font-bold text-foreground">{t('trips.title', 'Your Trips')}</Text>
            <TouchableOpacity
              onPress={() => router.push('/book-ride')}
              className="bg-primary px-5 py-3 rounded-2xl flex-row items-center gap-2 shadow-sm"
            >
              <Plus size={18} color="white" />
              <Text className="text-white font-bold">{t('trips.book', 'Book')}</Text>
            </TouchableOpacity>
          </View>
          ...
          <View className="space-y-4">
            {loading ? (
              ...
              <View className="items-center justify-center py-20 bg-card rounded-4xl border border-muted/10 border-dashed">
                ...
                <Text className="text-lg font-bold text-foreground">{t('trips.no_trips', 'No trips yet')}</Text>
                ...
                <View key={trip.id} className="bg-card p-6 rounded-3xl border border-muted/10 mb-5 shadow-sm">
                  <View className="flex-row justify-between items-start mb-6">
                    <View className="flex-row items-center gap-3">
                      ...
                      <View>
                        <Text className="font-bold text-foreground text-base">Trip Ride</Text>
                  ...
                  <View className="space-y-4">
                    <View className="flex-row items-start gap-4">
                      ...
                      <TouchableOpacity className="self-center bg-muted/10 p-3 rounded-2xl">
