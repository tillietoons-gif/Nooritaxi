...
export class FoodService {
  ...
  async updateOrder(id: string, data: any) {
    const { order, before } = await this.prisma.$transaction(async (tx) => {
      const before = await tx.order.findUnique({ where: { id }, include: { restaurant: true } });
      ...
      if (before && before.status !== 'READY_FOR_PICKUP' && order.status === 'READY_FOR_PICKUP' && !order.delivery) {
         const driver = await this.dispatch.findNearestOnlineDriver(order.restaurant.lat ?? 0, order.restaurant.lng ?? 0, 10, tx);
  ...
  private async settleDeliveredOrder(order: Order & { restaurant: { ownerId: string } }, tx: any) {
    ...
    await this.wallet.deposit(
      ...
      { tx, transactionType: 'MERCHANT_PAYOUT', description: `Merchant payout for order ${order.id}`, orderId: order.id },
    );
  }
}
