...
export default function BookingPage() {
  ...
  const [dropoffActiveIndex, setDropoffActiveIndex] = useState<number>(-1)
  ...
  function handlePickupKeyDown(event: React.KeyboardEvent<HTMLInputElement>) {
    ...
    switch (event.key) {
      ...
      case "ArrowUp":
        event.preventDefault()
        setPickupActiveIndex((prev) => (prev <= 0 ? pickupSuggestions.length - 1 : prev - 1))
        break
      case "Enter":
        if (pickupActiveIndex >= 0 && pickupActiveIndex < pickupSuggestions.length) {
          event.preventDefault()
  ...
  function handleDropoffKeyDown(event: React.KeyboardEvent<HTMLInputElement>) {
    ...
    switch (event.key) {
      ...
      case "ArrowUp":
        event.preventDefault()
        setDropoffActiveIndex((prev) => (prev <= 0 ? dropoffSuggestions.length - 1 : prev - 1))
        break
      case "Enter":
        if (dropoffActiveIndex >= 0 && dropoffActiveIndex < dropoffSuggestions.length) {
          event.preventDefault()
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="group relative">
                      ...
                      <Input
                        ...
                        aria-activedescendant={
                          pickupActiveIndex !== -1 ? `pickup-suggestion-${pickupActiveIndex}` : undefined
                        }
                      />
                          ...
                          {pickupSuggestions.map((place, index) => (
                            <button
                              ...
                              className={`block w-full px-3 py-2 text-left transition-colors hover:bg-muted ${
                                pickupActiveIndex === index ? "bg-muted" : ""
                              }`}
                    ...
                    <div className="group relative">
                      ...
                      <Input
                        ...
                        aria-activedescendant={
                          dropoffActiveIndex !== -1 ? `destination-suggestion-${dropoffActiveIndex}` : undefined
                        }
                      />
                        ...
                        >
                          {dropoffSuggestions.map((place, index) => (
                            <button
                              ...
                              tabIndex={-1}
                              className={`block w-full px-3 py-2 text-left transition-colors hover:bg-muted ${
                                dropoffActiveIndex === index ? "bg-muted" : ""
                              }`}
