...
export default function BookingPage() {
  ...
  const handlePickupKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === "ArrowDown") {
      ...
      setActivePickupIndex((prev) => (prev === pickupSuggestions.length - 1 ? 0 : prev + 1))
  ...
  const handleDropoffKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (dropoffSuggestions.length === 0) return

    if (event.key === "ArrowDown") {
      event.preventDefault()
      setActiveDropoffIndex((prev) => (prev === dropoffSuggestions.length - 1 ? 0 : prev + 1))
    } else if (event.key === "ArrowUp") {
      event.preventDefault()
              ...
              <Card className="border-none shadow-sm"><CardContent className="p-6 space-y-6">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="relative group">
                      ...
                      <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-primary transition-colors duration-200 group-focus-within:text-gold" />
                      <Input
                        id="pickup"
                        ...
                        aria-activedescendant={activePickupIndex !== -1 ? `pickup-item-${activePickupIndex}` : undefined}
                        onKeyDown={handlePickupKeyDown}
                      />
                      {pickupSuggestions.length > 0 ? (
                        <div id="pickup-listbox" role="listbox" aria-label="Pickup suggestions" className="absolute z-20 mt-2 w-full overflow-hidden rounded-lg border bg-background shadow-lg">
                          {pickupSuggestions.map((place, index) => (
                            <button
                              ...
                              tabIndex={-1}
                              className={cn(
                                "block w-full px-3 py-2 text-left hover:bg-muted transition-colors duration-150",
                                activePickupIndex === index && "bg-primary/10 text-primary font-semibold"
                              )}
                    ...
                    <div className="relative group">
                      ...
                      <Navigation className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-accent-foreground transition-colors duration-200 group-focus-within:text-gold" />
                      <Input
                        id="destination"
                        ...
                        aria-activedescendant={activeDropoffIndex !== -1 ? `destination-item-${activeDropoffIndex}` : undefined}
                        onKeyDown={handleDropoffKeyDown}
                      />
                      {dropoffSuggestions.length > 0 ? (
                        <div id="destination-listbox" role="listbox" aria-label="Destination suggestions" className="absolute z-20 mt-2 w-full overflow-hidden rounded-lg border bg-background shadow-lg">
                          {dropoffSuggestions.map((place, index) => (
                            <button
                              ...
                              className={cn(
                                "block w-full px-3 py-2 text-left hover:bg-muted transition-colors duration-150",
                                activeDropoffIndex === index && "bg-primary/10 text-primary font-semibold"
                              )}
